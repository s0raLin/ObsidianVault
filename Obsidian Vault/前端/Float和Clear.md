...
## `clear`用来做什么?
`clear`通常和`float`一起使用，用来消除浮动的影响

### `clear`的取值
* `none(default)`: 元素会按照前一个元素的`float`状态正常排列
* `left`: 如果前面的元素设置了`float: left`，当前元素就不会浮动到它的旁边
* `right`: 如果前面的元素设置了`float: right`，当前元素就不会浮动到它旁边
* `both`: 如果前一个元素使用了`float: right`或`float: right`，都会被清除
